...
export default {
  ...
  debounceFilter()
    ...
    const fetchUsersList = async () => {
      try {
        const response = await axios.get('https://hr.ultimateit.io/api/Account/GetAllUsers')
        ...
        if (result.isConfirmed) {
          try {
            const response = await axios.delete(
              `https://hr.ultimateit.io/api/Account/DeleteUser/${id}`
            )
            Swal.fire({
              icon: 'success',
              title: 'Deleted',
    ...
    const submit = async () => {
      await fetch('https://hr.ultimateit.io/api/Account/register', {
        method: 'POST',
