...
export default {
  ...
  setup() {
    ...
    const fetchJobApplicationsList = async () => {
      try {
        const response = await axios.get(
          'https://hr.ultimateit.io/api/Candidature/GetCandidaturesByUSer',
          {
            headers: {
              Authorization: `Bearer ${jwtToken}`
            },
    ...
    const downloadCv = async (cvFileName: string) => {
      await axios
        .get(`https://hr.ultimateit.io/api/Candidature/download/${cvFileName}`, {
          responseType: 'blob'
        })
        .then((response) => {
        ...
        if (result.isConfirmed) {
          try {
            ...
            const response = await axios.delete(
              `https://hr.ultimateit.io/api/Candidature/Delete/${appUserId}/${offreId}`,
              {
                headers: {
                  Authorization: `Bearer ${jwtToken}`
                },
